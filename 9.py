...
def IntCode(code,instream,outstream):
    ...
    while True:
        ...
        if op==99:
            ...
            code[getPosCode(mode[3],i+3,offset,code)]=code[getPosCode(mode[2],i+2,offset,code)]+code[getPosCode(mode[1],i+1,offset,code)]
            i+=4
        elif op==2:
            code[getPosCode(mode[3],i+3,offset,code)]=code[getPosCode(mode[2],i+2,offset,code)]*code[getPosCode(mode[1],i+1,offset,code)]
            i+=4
            ...
            code[getPosCode(mode[1],i+1,offset,code)]=ipt
            i+=2
        elif op==4:
            lock.acquire()
            inputs[outstream]+=[code[getPosCode(mode[1],i+1,offset,code)]]
            #print(inputs)
            lock.release()
            i+=2
            
        elif op==5:
            if code[getPosCode(mode[1],i+1,offset,code)]!=0:
                i=code[getPosCode(mode[2],i+2,offset,code)]
            else:
                i+=3
        elif op==6:
            if code[getPosCode(mode[1],i+1,offset,code)]==0:
            ...
            if code[getPosCode(mode[1],i+1,offset,code)]<code[getPosCode(mode[2],i+2,offset,code)]:
                code[getPosCode(mode[3],i+3,offset,code)]=1
            else :
                code[getPosCode(mode[3],i+3,offset,code)]=0
            i+=4    
        elif op==8:
            if code[getPosCode(mode[1],i+1,offset,code)]==code[getPosCode(mode[2],i+2,offset,code)]:
                code[getPosCode(mode[3],i+3,offset,code)]=1
            ...
            offset+=code[getPosCode(mode[1],i+1,offset,code)]
            #print(offset)
            i+=2
        else:
